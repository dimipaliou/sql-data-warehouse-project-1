...
Script Purpose:
  ...
  Actions performed:
    ...
    -Inserts transformed and cleansed data from Bronze into Silver schema
...
BEGIN
	...
	SET @batch_start_time=GETDATE();
		...
		SELECT 
			...
			CASE WHEN TRIM(UPPER(cst_marital_status))='S' THEN 'Sinlge'
		...
		PRINT '------------------------------';
			/*    crm_prd_info     */
		--============================================================
		--the transformation requires new columns and data types, therefore we will reuse the DDL query to update the talbe before loading the data
		--Load the data with final query
		SET @start_time=GETDATE();
		...
		SELECT
		sls_ord_num, /*no transformation*/
		sls_prd_key, /*no transformation*/
		sls_cust_id, /*no transformation*/
		...
		PRINT '>> Inserting Data Info: silver.erp_loc_101';
